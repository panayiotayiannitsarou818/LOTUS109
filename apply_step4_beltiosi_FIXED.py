...
def infer_col_and_classes(df, preferred):
    """
    Find the step3 assignment column and infer class labels.
    If no explicit 'Α\d+' labels exist, compute k = ceil(N/25) and create ['Α1', ..., 'Αk'].
    """
    ...
    if not classes:
        n = compute_num_classes(len(df), max_per_class=25, min_classes=2)
        classes = make_class_labels(n, prefix='Α')
    return col, classes
