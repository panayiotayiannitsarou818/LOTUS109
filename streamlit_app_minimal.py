    ...
    for scenario_name, step1_data in step1_results.items():
        ...
        try:
            ...
            step2_results = step2_apply_FIXED_v3(
                df, num_classes=num_classes, step1_col_name=step1_col, max_results=1
            )
            ...
            df, step3_metrics = apply_step3_on_sheet(df, step2_col, num_classes=num_classes)
            ...
            step4_results = apply_step4_strict(
                df, assigned_column=step3_col, num_classes=num_classes, max_results=1, max_nodes=20000
            )
            ...
            df_step5, penalty5 = apply_step5_to_all_scenarios(
                {scenario_name: df}, step4_col, num_classes=num_classes
            )
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.data is not None:
            ...
            if 'final' in st.session_state.step_results:
                ...
                # Επιλογή σεναρίου για εξαγωγή
                scen_names = list(st.session_state.step_results['final'].keys())
                scen_default = 0
                scen_choice = st.sidebar.selectbox("Σενάριο για εξαγωγή", scen_names, index=scen_default)
                scen_num_match = re.search(r"(\d+)$", scen_choice)
                scen_num = int(scen_num_match.group(1)) if scen_num_match else 1
                the_df = st.session_state.step_results['final'][scen_choice]['df']
                final_col = st.session_state.step_results['final'][scen_choice].get('final_column')
                buf2 = export_analytika_vimata_excel(the_df, scenario_number=scen_num, final_column=final_col)
                st.sidebar.download_button(
