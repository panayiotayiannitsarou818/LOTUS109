...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(f"✅ Δεδομένα διαβάστηκαν:

# Auto num_classes = ceil(N/25)
N = len(df)
k = compute_num_classes(N)
st.session_state['num_classes'] = k
st.metric("Προτεινόμενα τμήματα (k)", k)
st.write("Ετικέτες τμημάτων:", make_class_labels(k))
try:
    with open("num_classes.json", "w", encoding="utf-8") as f:
        json.dump({"num_classes": k}, f, ensure_ascii=False)
    st.caption("Αποθηκεύτηκε num_classes.json (για άλλα modules)")
except Exception:
    pass {len(df)} γραμμές, {len(df.columns)} στήλες")
