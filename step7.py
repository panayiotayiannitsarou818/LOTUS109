...
5) ΠΡΟΣΘΗΚΗ: Wrapper function run_step7() για consistency
...
# ------------------------ ΠΡΟΣΘΗΚΗ: Wrapper Function ------------------------

def run_step7(df: pd.DataFrame, scenario_cols: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ΠΡΟΣΘΗΚΗ: Wrapper function για Βήμα 7 - Τελική βαθμολόγηση σεναρίων
    
    Args:
        df: DataFrame με δεδομένα μαθητών
        scenario_cols: Lista στηλών σεναρίων για αξιολόγηση (auto-detect αν None)
    
    Returns:
        Dict με αποτελέσματα βαθμολόγησης και βέλτιστο σενάριο
    """
    df = df.copy()
    
    # Auto-detect scenario columns αν δεν δόθηκαν
    if scenario_cols is None:
        scenario_cols = []
        for c in df.columns:
            if re.match(r"^ΒΗΜΑ6_ΣΕΝΑΡΙΟ_\d+__1$", str(c)):
                scenario_cols.append(c)
    ...
    if not scenario_cols:
        return {
            "success": False,
            "message": "Δεν βρέθηκαν κατάλληλες στήλες σεναρίων για αξιολόγηση",
            "best_scenario": None,
            "scores": []
        }
    
    # Normalize class labels και ensure optional columns
    for col in scenario_cols:
        if col in df.columns:
            _normalize_class_labels(df, col)
    _ensure_optional_cols(df)
    
    try:
        # Βρες το βέλτιστο σενάριο
        result = pick_best_scenario(df, scenario_cols, random_seed=RANDOM_SEED)
        
        # Δημιούργησε summary DataFrame
        scores_df = score_to_dataframe(df, scenario_cols)
        
        return {
            "success": True,
            "message": f"Αξιολογήθηκαν {len(scenario_cols)} σενάρια επιτυχώς",
            "best_scenario": result["best"],
            "all_scores": result["scores"],
            "scores_summary": scores_df.to_dict('records'),
            "num_scenarios": len(scenario_cols),
        ...
        return {
            "success": False,
            "message": f"Σφάλμα κατά την αξιολόγηση: {str(e)}",
            "best_scenario": None,
            "scores": []
        }


# ======================= NEW: Per-class Excel Exporter =======================
def export_final_by_class_excel(df: pd.DataFrame, final_col: str, out_path: str,
                                sort_names: bool = True, include_summary: bool = True) -> str:
    """
    Δημιουργεί Excel με ΕΝΑ φύλλο ανά τμήμα, που περιέχει μόνο τις στήλες:
    - Α/Α (σειριακή αρίθμηση)
    - ΟΝΟΜΑ
    """
    if "ΟΝΟΜΑ" not in df.columns:
        raise ValueError("Λείπει η στήλη 'ΟΝΟΜΑ' από το DataFrame.")
    if final_col not in df.columns:
        raise ValueError(f"Λείπει η στήλη '{final_col}' από το DataFrame.")

    data = df.copy()
    data[final_col] = data[final_col].astype(str)

    classes = sorted([c for c in data[final_col].dropna().unique() if str(c).strip()])
    if not classes:
        raise ValueError("Δεν βρέθηκαν τιμές τμημάτων στη στήλη final_col.")

    # Γράψιμο με openpyxl για μέγιστη συμβατότητα
    with pd.ExcelWriter(out_path, engine="openpyxl") as wr:
        if include_summary:
            counts = (data.groupby(final_col, dropna=False)
                           .size().reset_index(name="ΠΛΗΘΟΣ")
                           .sort_values(final_col))
            counts.to_excel(wr, index=False, sheet_name="Σύνοψη")
        for cls in classes:
            df_cls = data[data[final_col] == cls][["ΟΝΟΜΑ"]].copy()
            if sort_names:
                df_cls = df_cls.sort_values("ΟΝΟΜΑ").reset_index(drop=True)
            df_cls.insert(0, "Α/Α", range(1, len(df_cls)+1))
            df_cls.to_excel(wr, index=False, sheet_name=str(cls))
    return out_path


def run_step7_and_export_by_class(combined_df: pd.DataFrame, scenario_cols: list[str],
                                  out_scores_path: str, out_by_class_path: str) -> dict:
    """
    Υπολογίζει βαθμολογίες, επιλέγει καλύτερο σενάριο και δημιουργεί:
      (α) Scores.xlsx (ή φύλλο Scores) και
      (β) Excel «ένα φύλλο ανά τμήμα» με τα ονόματα.
    Επιστρέφει dict με πληροφορίες, συμπεριλαμβανομένης της στήλης best_col.
    """
    scores_df = score_to_dataframe(combined_df, scenario_cols)
    best = pick_best_scenario(combined_df, scenario_cols)

    # Φτιάξε Final dataframe με την καλύτερη στήλη
    best_col = best["best"]["scenario_col"] if best and best.get("best") else scenario_cols[0]
    final_df = combined_df[["ΟΝΟΜΑ"]].copy()
    final_df["ΤΕΛΙΚΟ_ΤΜΗΜΑ"] = combined_df[best_col]

    # (α) Scores + Final για συμβατότητα
    with pd.ExcelWriter(out_scores_path, engine="openpyxl") as wr:
        scores_df.to_excel(wr, index=False, sheet_name="Scores")
        final_df.to_excel(wr, index=False, sheet_name="Final")

    # (β) Ένα φύλλο ανά τμήμα (μόνο ονόματα)
    export_final_by_class_excel(final_df, "ΤΕΛΙΚΟ_ΤΜΗΜΑ", out_by_class_path, sort_names=True, include_summary=True)

    return {
        "best_col": best_col,
        "scores_path": out_scores_path,
        "by_class_path": out_by_class_path,
        "num_scenarios": len(scenario_cols)
    }
# ===================== END NEW: Per-class Excel Exporter =====================



if __name__ == "__main__":
    # CLI: python step7.py [STEP7_FINAL.xlsx] [final_by_class.xlsx]
    import sys
    in_path = "STEP7_FINAL.xlsx" if len(sys.argv) < 2 else sys.argv[1]
    out_path = "final_by_class.xlsx" if len(sys.argv) < 3 else sys.argv[2]
    import pandas as pd
    df = pd.read_excel(in_path, sheet_name="Final")
    export_final_by_class_excel(df, "ΤΕΛΙΚΟ_ΤΜΗΜΑ", out_path)
    print(out_path)
